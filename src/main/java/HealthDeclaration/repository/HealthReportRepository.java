...
public interface HealthReportRepository extends JpaRepository<HealthReport, Long> {
    ...
    @Query(" SELECT new HealthDeclaration.modal.dto.HealthReportDTO(hr.id,hr.createdBy,hr.createdTime,hr.modifiedBy,hr.modifiedTime," +
            ...
            "hr.contactToPlace,hr.sicking,prv.name,dis.name,wa.name,hr.factor)  FROM HealthReport hr  "
            ...
            +"  WHERE hr.username = :username AND hr.createdTime >= :dateTime ")
    List<HealthReportDTO> getReportsByUsername(@Param("username") String username ,@Param("dateTime") Date dateTime);
}
